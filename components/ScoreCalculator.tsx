...
const ScoreCalculator: React.FC = () => {
  ...
  const calculateScore = () => {
      ...
      if (activeScore === 'lysholm') {
          ...
          finalScore = totalPoints / currentQuestions.length;
      }
  ...
  const getInterpretation = (val: number) => {
      ...
      if (val >= 80) return { label: 'Bom', color: 'text-blue-600', stroke: '#2563eb', bg: 'bg-blue-100', msg: 'Limitações leves no esporte/dia a dia.' };
      ...
      return { label: 'Ruim', color: 'text-red-600', stroke: '#dc2626', bg: 'bg-red-100', msg: 'Disfunção articular significativa.' };
  };
          ...
          <div className="h-full bg-slate-50 flex flex-col pb-24 lg:pb-0 animate-fadeIn">
              ...
              <div className="sticky top-0 bg-white/90 backdrop-blur-md z-10 px-4 py-3 border-b border-slate-200 flex justify-between items-center shadow-sm">
                  ...
                  <div className="text-xs font-bold text-slate-400">
                      {Object.keys(inputs).length} / {activeQuestions.length}
                  </div>
              </div>
                  ...
                  {result !== null && interpretation ? (
                      <div className="animate-scaleIn max-w-sm mx-auto mt-6">
                          <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl p-8 text-center relative overflow-hidden">
                              ...
                              <div className="relative w-48 h-48 mx-auto mb-6">
                                  <svg className="w-full h-full transform -rotate-90">
                                      ...
                                      <circle 
                                          ...
                                          fill="transparent" 
                                          strokeDasharray="552" 
                                          strokeDashoffset={552 - (552 * result) / 100} 
                                          strokeLinecap="round"
                              ...
                              <button 
                                onClick={reset}
                                className="mt-8 w-full py-4 bg-slate-900 text-white rounded-xl font-bold text-sm hover:bg-slate-800 transition-all active:scale-95 shadow-lg"
                              >
                                  Novo Cálculo
                              </button>
                          </div>
                      ...
                      <div className="space-y-4 max-w-2xl mx-auto pb-8">
                          {activeQuestions.map((q, idx) => (
                              <div key={q.id} className="bg-white p-5 rounded-3xl border border-slate-100 shadow-sm animate-slideUp" style={{ animationDelay: `${idx * 0.05}s` }}>
                                  <h3 className="font-bold text-slate-900 text-sm mb-4 flex gap-2">
                                      <span className="bg-slate-100 text-slate-500 px-2.5 py-0.5 rounded-md text-xs h-fit font-mono">{idx + 1}</span>
                                      {q.title}
                                  </h3>
                                  <div className="space-y-2.5">
                                      {q.options.map((opt) => (
                                          <button
                                              ...
                                              className={`w-full text-left p-4 rounded-xl text-xs font-bold transition-all border flex items-center justify-between active:scale-[0.98]
                                              ${inputs[q.id] === opt.v 
                                                  ? 'bg-slate-900 text-white border-slate-900 shadow-md ring-1 ring-slate-900' 
                                                  : 'bg-slate-50 text-slate-600 border-transparent hover:bg-slate-100'}`}
                                          >
                                              {opt.l}
                                              {inputs[q.id] === opt.v && <CheckCircle className="w-4 h-4 text-white" />}
                                          </button>
                                      ))}
                                  </div>
                              </div>
                          ...
                          <div className="pt-4">
                              <button 
                                  ...
                                  className="w-full bg-gradient-to-r from-blue-600 to-indigo-600 text-white py-4 rounded-2xl font-bold text-lg shadow-xl shadow-blue-500/30 flex items-center justify-center gap-3 active:scale-[0.98] transition-all hover:brightness-110"
                              >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {scores.map((score) => (
                        <button 
                            ...
                            className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm hover:shadow-lg hover:border-blue-200 transition-all text-left group active:scale-[0.98] relative overflow-hidden"
                        >
